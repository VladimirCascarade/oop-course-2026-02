"""A metaclass for inheritance tree structures."""
...
class ClassTreeMeta(ABCMeta):
    """A metaclass for inheritance tree structures."""

    __parent_class: ClassTreeMeta | None
    __root_class: ClassTreeMeta
    ...
    def __new__(
        ...
        parent_class: ClassTreeMeta | None = None
        for base in bases:
            if isinstance(base, ClassTreeMeta):
                if parent_class is not None:
                    raise TypeError(
                        "Class of metaclass ClassTreeMeta can have at most"
                        " one base class of metaclass ClassTreeMeta."
                    )
                parent_class = base
    ...
    @property
    def _parent_class(self) -> ClassTreeMeta | None:
        """
    ...
    def _subclass_join[_T: type](cls: _T, subclasses: Iterable[_T]) -> _T | None:
        ...
        of this class.
        """
        join = type(cast(ClassTreeMeta, cls))._join(
            cast(Iterable[ClassTreeMeta], subclasses)
        )
        if join is None:
            return None
